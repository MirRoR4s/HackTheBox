...
上图在一个非常高的层次给出了HTTP请求的组成。首先用户在浏览器输入URL，浏览器会向DNS服务器发起一个请求以解析域名并得到其ip地址。DNS服务器会查询inlanefreight.com的IP地址并返回。所有的域名都需要通过这种方式解析，因为服务器端在没有IP地址的情况下无法通信。

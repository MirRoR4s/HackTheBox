---
description: https://academy.hackthebox.com/module/18/section/2097
---
...
```shell-session
MirRoR4s@htb[/htb]$ docker build -t FS_docker
```
...
```shell-session
MirRoR4s@htb[/htb]$ docker run -p <host port>:<docker port> -d <docker container name>
```
...
```shell-session
MirRoR4s@htb[/htb]$ docker run -p 8022:22 -p 8080:80 -d FS_docker
```
...
```shell-session
MirRoR4s@htb[/htb]$ sudo apt-get install lxc lxc-utils -y
```
...
```shell-session
MirRoR4s@htb[/htb]$ sudo lxc-create -n linuxcontainer -t ubuntu
```
...
```shell-session
MirRoR4s@htb[/htb]$ sudo vim /usr/share/lxc/config/linuxcontainer.conf
```
...
```shell-session
MirRoR4s@htb[/htb]$ sudo systemctl restart lxc.service
```

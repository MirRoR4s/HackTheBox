...
```shell-session
MirRoR4s@htb[/htb]$ cat /etc/fstab
...
```shell-session
MirRoR4s@htb[/htb]$ sudo mount /dev/sdb1 /mnt/usb
MirRoR4s@htb[/htb]$ cd /mnt/usb && ls -l
...
```shell-session
MirRoR4s@htb[/htb]$ sudo umount /mnt/usb
```
...
&#x20; Full Screen  Terminate   Reset Life Left: 47mConnected to htb-gt4iwglkm0:1 (htb-ac-748864)

**Questions**

Answer the question(s) below to complete this Section and earn cubes!

Cheat Sheet+ 0  What is the size in GiB of the "/dev/sda" disk in our Pwnbox? (Format: 000)

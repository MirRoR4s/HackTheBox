...
```shell-session
MirRoR4s@htb[/htb]$ cat /etc/ssh/sshd_config | grep X11Forwarding
...
```shell-session
MirRoR4s@htb[/htb]$ ssh -X htb-student@10.129.23.11 /usr/bin/firefox
...
```shell-session
MirRoR4s@htb[/htb]$ ssh -L 5901:127.0.0.1:5901 -N -f -l htb-student 10.129.14.130
...
```shell-session
MirRoR4s@htb[/htb]$ xtightvncviewer localhost:5901

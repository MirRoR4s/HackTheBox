...
```bash
// Some code
1. cat /etc/ssh/sshd_config | grep -E "~#"
2. cat /etc/ssh/sshd_config | grep -E "^([^A-Za-z]*)Permit([[:alpha:]]*)"
3. cat /etc/ssh/sshd_config | grep -E "[A-Za-z]*(Authentication$)"
4. cat /etc/ssh/sshd_config | grep -E "Key"
5. cat /etc/ssh/sshd_config | grep -E "(Password).*yes"
6. cat /etc/ssh/sshd_config | grep -E "yes$"
